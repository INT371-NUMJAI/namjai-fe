<template>
      ...
      <div class="lg:max-w-sm md:max-w-md rounded-lg overflow-hidden bg-namjaiwhite drop-shadow-md hover:shadow-md cursor-pointer">
        ...
        <div class="lg:px-6 lg:py-4 px-4 py-3 md:py-5">
          <div class="flex justify-between items-center">
            <h2 class="font-medium text-lg mb-1 overflow-hidden truncate w-[180px] md:[20px]">{{ volunteerProp.volunteerProjectName }}</h2>
          ...
          <div class="mb-4 text-sm">{{ volunteerProp.foundationName }}</div>
          <div class="space-y-3">
            <div class="flex space-x-3">
              <w-icon color="namjaigreen">fa fa-map-marker</w-icon>
              <p class="text-sm">{{ volunteerProp.locationProvince }}</p>
            </div>
            <div class="flex space-x-3">
              <w-icon color="namjaigreen">fa fa-calendar-o</w-icon>
              <p class="text-sm">{{ volunteerProp.activityStartDate.slice(0, 10) }} ถึง {{ volunteerProp.activityEndDate.slice(0, 10) }}</p>
            </div>
            <div class="flex space-x-3">
              <w-icon class="" color="namjaigreen">fa fa-user-o</w-icon>
              <p class="text-sm">{{ volunteerProp.peopleRegistered }}/{{ volunteerProp.peopleNeeded }}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  <div class="py-[60px] flex justify-center">
    <button @click="loadMore()" v-if="(currentPage * maxPerPage < volunteerProps.length) && searchText === ``" class="bg-transparent hover:bg-namjaigreen text-namjaigreen font-medium hover:text-white py-2 px-10 border-2 border-namjaigreen hover:border-transparent rounded">เพิ่มเติม</button>
  </div>
</template>
...
export default {
  ...
  setup(props) {
    ...
    const searchVolunteerList = computed(() => {
      isFound.value = false;
      if (searchText.value === "") {
        return props.volunteerProps.slice(0, currentPage.value * maxPerPage.value)
      } else {
