        ...
        <div v-if="showManage" class="absolute mt-2 -ml-4 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu">
          <div class="py-1" role="none">
            <router-link to="/admin-management">
              <div class="text-gray-700 block text-right px-4 py-2 text-sm cursor-pointer">มูลนิธิ</div>
            </router-link>
            <div class="text-gray-700 block text-right px-4 py-2 text-sm cursor-pointer">คำร้องเรียน</div>
          </div>
        </div>
      </div>
