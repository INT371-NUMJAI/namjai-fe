  ...
  >
    <div class="flex items-center text-white space-x-5">
      ...
      <h1
        class="mt-2"
        v-if="
          use_auth.auth_role.value === `ADMIN` &&
    ...
    <div
      class="hidden lg:flex lg:space-x-11 items-center text-white lg:text-base"
    >
      <div name="manage" class="container inline-block">
        ...
        >
          ...
          <div class="ml-2 -mt-[2px]">
            <w-icon md color="white" v-if="showManage == false"
              >fa fa-caret-down</w-icon
            >
        ...
        <div
          v-if="showManage"
          class="absolute mt-2 -ml-4 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
          role="menu"
        >
          <div class="text-gray-700 text-center text-sm" role="none">
            <div
              class="block rounded-t-md px-4 py-2 cursor-pointer hover:bg-gray-200"
              @click="routeToManageFDN"
            >
              มูลนิธิ
            </div>
            <div
              class="block rounded-b-md px-4 py-2 cursor-pointer hover:bg-gray-200"
            >
              คำร้องเรียน
            </div>
          </div>
      ...
      <div>
        ...
        <span v-if="use_auth.store_auth.user !== null">
          ...
          >
            ...
            <div class="ml-2 -mt-[2px]">
              <w-icon md color="white" v-if="showDropDown == false"
                >fa fa-caret-down</w-icon
              >
              <w-icon md color="white" v-if="showDropDown"
                >fa fa-caret-up</w-icon
              >
            </div>
          </w-button>
          <div>
            ...
            >
              <div class="text-gray-700 text-center text-sm">
                ...
                <div
                  class="block rounded-b-md px-4 py-2 cursor-pointer text-red-600 hover:bg-gray-200"
                  @click="clicktoLogout"
                >
                  ลงชื่อออก
                </div>
