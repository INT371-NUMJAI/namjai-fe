<template>
  ...
  >
    <div class="lg:col-span-8 lg:mt-[120px]">
      <div
        class="grid grid-flow-col mb-[20px] text-sm lg:text-base font-medium"
      >
        <div class="flex space-x-2">
          <p>หมวดหมู่:</p>
          <p
            class="text-namjaigreen"
            v-for="fdnProjectProp in project.targetCategoriesSet"
            :key="fdnProjectProp"
          >
      ...
      <img
        class="w-full aspect-video object-cover my-5 lg:my-[30px]"
        src="@/assets/pic1.png"
      />
      <div class="bg-white mt-[30px] lg:mt-[40px]">
        <div
          class="flex justify-center md:justify-start lg:justify-start space-x-8 md:space-x-12 lg:space-x-12 text-sm lg:text-base md:px-8 lg:px-10"
        >
          <div
            class="flex-wrap space-y-3 pt-4 lg:pt-6 h-[54px] lg:h-[66px]"
            @click="
              (selectedDetail = true),
                (selectedProgression = false),
                (selectedFinancialPlan = false)
            "
          >
            <h2 class="cursor-pointer select-all selection:text-namjaigreen">
              รายละเอียด
            </h2>
            <div
              class="h-1.5 bg-emerald-600"
              v-if="selectedDetail === true"
            ></div>
          </div>
          <div
            class="flex-wrap space-y-3 pt-4 lg:pt-6 h-[54px] lg:h-[66px]"
            @click="
              (selectedProgression = true),
          ...
          >
            ...
            <div
              class="h-1.5 bg-emerald-600"
              v-if="selectedProgression === true"
            ></div>
          </div>
          <div
            class="flex-wrap space-y-3 pt-4 lg:pt-6 h-[54px] lg:h-[66px]"
            @click="
              (selectedFinancialPlan = true),
                (selectedDetail = false),
                (selectedProgression = false)
            "
          >
            ...
            <div
              class="h-1.5 bg-emerald-600"
              v-if="selectedFinancialPlan === true"
            ></div>
          </div>
        </div>
      </div>
      ...
      <div
        name="content"
        class="bg-white text-sm lg:text-base rounded-b-md mb-[30px] lg:mt-0 lg:mb-[40px]"
      >
    ...
    <div
      name="donate"
      class="lg:col-span-4 lg:h-full bg-white text-sm lg:text-base rounded-md lg:rounded-none block lg:flex lg:justify-center"
    >
      <div class="p-5 lg:mx-auto md:p-8 lg:my-24 lg:py-7 lg:px-0 lg:fixed">
        <div class="hidden lg:block">
              ...
              <w-icon
                v-if="
                  ...
                  project.foundationContactDTO.email ===
                    use_auth.store_auth.user.email
                "
          ...
          <h1 class="text-xl mb-[30px] lg:w-[300px]">
            {{ project.foundationProjectName }} ({{ project.status }})
          </h1>
        </div>
        ...
        <w-form v-model="valid">
          ...
          <w-dialog v-if="showDialog" :width="400">
            ...
            <credit-card
              @closeThisComp="close"
              :amountProp="Number(amount)"
              :projectName="project.foundationProjectName"
              :foundationProject="project.foundationProjectUUID"
              :validForShow="valid"
            />
          </w-dialog>
        </w-form>
        <div name="shareDt" class="hidden lg:block pt-16">
          ...
          <div class="flex space-x-3">
            <a :href="linkShare">
              <svg
                class="fill-current hover:text-[#0165E1] active:text-[#0165E1] cursor-pointer"
                width="38"
                height="38"
                viewBox="0 0 38 38"
                fill="none"
            ...
            <a :href="linkShareTwitter">
              <svg
                class="fill-current hover:text-[#17A9FD] active:text-[#17A9FD] cursor-pointer"
                width="38"
                height="38"
                viewBox="0 0 38 38"
                fill="none"
  ...
  setup() {
    ...
    const copyToClipboard = () => {
      navigator.clipboard.writeText(
        `https://namjai.site/project/${route.params.id}`
      );
    };
