    ...
    <div class="mt-[120px] mx-28">
				...
				<fieldset class="border border-namjaibrown rounded-md px-[58px] mb-[60px]">
					...
					<div class="mb-[20px] space-y-[20px]">
						...
						<p>ซัพพลายเออร์พ่อค้าอีสเตอร์ไฮเวย์ ดีพาร์ทเมนท์วาฟเฟิลโมหจริตซีนีเพล็กซ์ เยอบีราเพียบแปร้ เทวา
							วานิลาแมกกาซีนเป่ายิงฉุบ สี่แยกริคเตอร์ อิเลียดร็อครายชื่อชนะเลิศเจล
							โอวัลตินแพทยสภาคาแรคเตอร์เอ๊าะ แจม เซ็นเตอร์ศากยบุตรวัคค์เมจิกไลน์ เย้ว
							บ๊อบอัลมอนด์ครัวซองต์ เป่ายิงฉุบคาปูชิโน การันตีพาวเวอร์โปรเจ็กต์ อุด้ง
							ภคันทลาพาธระโงกสงบสุข

							รามาธิบดีทาวน์เฮาส์แกงค์กุมภาพันธ์ เปเปอร์กับดักเคลียร์ถูกต้อง สแล็กเฟิร์มโปรดักชั่นแทงกั๊ก
							เฮีย เฝอ แฟล็ตแมชีนมหภาค คำสาปอาว์คอนเซปต์แทงโก้ ทำงานแกสโซฮอล์แพกเกจช็อตเคส แคนู
							ฮองเฮานายพรานพะเรอซาบะ พุทธศตวรรษเยอบีราแอปพริคอท โยโย่บอมบ์ปฏิสัมพันธ์สกรัม เทรดโกลด์
							แซลมอนเย้ว ดีกรีเลดี้สุนทรีย์โยโย่ฮาราคีรี ไตรมาสก่อนหน้า</p>
					</div>
				...
				<div class="flex mb-[60px] mx-auto justify-center space-x-[50px]">
					...
					<div class="flex">
						<base-button @click="dialog.show = true"
							class="bg-namjaired w-[150px] h-[50px] justify-start" />
					</div>
				</div>
				<w-dialog v-model="dialog.show" :width="dialog.width" bg-color="namajai-green">
					<template #title bg-color="namjai-beige">
						...
						<w-button class="button--close mt-1 mx-2" @click="dialog.show = false" sm outline round absolute
							color="namjai-red" icon="wi-cross"></w-button>
					</template>
					<w-textarea color="black" placeholder="รายละเอียดที่ไม่อนุมัติ"></w-textarea>
					<template #actions>
						<base-button @click="dialog.show = false" class="h-[40px] text-xs" />
						<!-- <w-button bg-color="namjai-green" color="white" @click="dialog.show = false">ยืนยัน</w-button> -->
					</template>
				</w-dialog>
			</div>
</template>
